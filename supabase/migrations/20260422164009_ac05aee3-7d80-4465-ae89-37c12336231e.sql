-- Extend get_app_stats with USD totals + combined videos field (backward compatible: keep all existing columns)
...
RETURNS TABLE(
  ...
  total_videos_combined bigint,
  total_sent_usd numeric,
...
BEGIN
  ...
  -- Combined: all non-gift posts + ended livestreams (gộp 4 ô thành 1)
  ...
  -- USD totals: donations + wallet_transfers + swaps (reference price table)
  WITH price_of AS (
    SELECT * FROM (VALUES
      ('BTC', 95000.0), ('BTCB', 95000.0), ('WBTC', 95000.0),
...
-- Breakdown per token, dùng cho modal Tổng tặng / Tổng nhận
CREATE OR REPLACE FUNCTION public.get_global_gift_breakdown(p_direction text)
...
GRANT EXECUTE ON FUNCTION public.get_global_gift_breakdown(text) TO anon, authenticated;