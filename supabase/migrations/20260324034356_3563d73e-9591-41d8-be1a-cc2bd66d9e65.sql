...
CREATE OR REPLACE FUNCTION public.get_user_honor_stats(p_user_id uuid)
 RETURNS TABLE(posts_count bigint, reactions_on_posts bigint, comments_count bigint, shares_count bigint, friends_count bigint, livestreams_count bigint, total_reward numeric, today_reward numeric, claimed_amount numeric, actual_posts_count bigint, actual_reactions_count bigint, actual_comments_count bigint, actual_shares_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM posts WHERE user_id = p_user_id AND (post_type IS NULL OR post_type <> 'gift_celebration');
