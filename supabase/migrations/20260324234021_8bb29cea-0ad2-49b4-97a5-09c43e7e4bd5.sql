...
-- Drop and recreate get_user_posts_by_period_vn with correct return type
...
BEGIN
  ...
  ELSIF p_type = 'livestreams' THEN
    RETURN QUERY
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE c.live_session_id = ls.id AND c.status = 'completed' AND c.output_url IS NOT NULL
  ...
  ELSIF p_type = 'rewards' THEN
    RETURN QUERY
      SELECT rc.id,
             ('Rút ' || rc.amount::text || ' CAMLY')::text as content,
             NULL::text as image_url,
             NULL::text as video_url,
             NULL::jsonb as media_urls,
             'reward_claim'::text as post_type,
             rc.created_at
...
GRANT EXECUTE ON FUNCTION public.get_user_posts_by_period_vn TO anon, authenticated, service_role;
