...
-- Sync get_app_stats with same filtering logic as the other 3 functions
...
BEGIN
  ...
  -- Posts: exclude gift_celebration (synced with get_content_stats_grouped_vn)
  SELECT COUNT(*) INTO v_total_posts FROM posts
  WHERE (post_type IS NULL OR post_type NOT IN ('gift_celebration'));

  -- Photos: has image/media, no video, exclude video/gift types (synced)
  ...
  RETURN QUERY SELECT v_total_users, v_total_posts, v_total_photos, v_total_videos, v_total_livestreams, v_total_rewards, v_treasury, v_claimed;
END;
$$;
