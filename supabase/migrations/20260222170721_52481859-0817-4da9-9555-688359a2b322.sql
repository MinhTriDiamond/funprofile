...
BEGIN
  ...
  old_stats AS (
    SELECT 
      p.id AS user_id,
      COALESCE((SELECT COUNT(*) FROM posts pt WHERE pt.user_id = p.id AND pt.created_at < cutoff_date AND COALESCE(pt.is_reward_eligible, true) = true), 0) AS old_posts,
      ...
      COALESCE((SELECT COUNT(*) FROM friendships fr WHERE (fr.user_id = p.id OR fr.friend_id = p.id) AND fr.status = 'accepted' AND fr.created_at < cutoff_date), 0) AS old_friends
    FROM profiles p
  ),
  new_daily_posts AS (
  ...
  new_daily_friends AS (
    SELECT nf.user_id, nf.reward_date, LEAST(COUNT(*), 10) AS capped_count FROM (
      SELECT fr.user_id, (fr.created_at AT TIME ZONE 'UTC')::DATE AS reward_date FROM friendships fr WHERE fr.status = 'accepted' AND fr.created_at >= cutoff_date
      UNION ALL
      SELECT fr.friend_id AS user_id, (fr.created_at AT TIME ZONE 'UTC')::DATE AS reward_date FROM friendships fr WHERE fr.status = 'accepted' AND fr.created_at >= cutoff_date
    ) nf GROUP BY nf.user_id, nf.reward_date
  ),
  new_daily_livestreams AS (
    SELECT l.user_id, (l.started_at AT TIME ZONE 'UTC')::DATE AS reward_date, LEAST(COUNT(*), 5) AS capped_count
    FROM livestreams l WHERE l.started_at >= cutoff_date AND l.is_eligible = true
    GROUP BY l.user_id, (l.started_at AT TIME ZONE 'UTC')::DATE
  ),
  new_stats AS (
    SELECT 
      p.id AS user_id,
      COALESCE((SELECT SUM(ndp.capped_count) FROM new_daily_posts ndp WHERE ndp.user_id = p.id), 0) AS new_posts,
      COALESCE((SELECT SUM(ndr.capped_count) FROM new_daily_reactions ndr WHERE ndr.user_id = p.id), 0) AS new_reactions,
      COALESCE((SELECT SUM(ndc.capped_count) FROM new_daily_comments ndc WHERE ndc.user_id = p.id), 0) AS new_comments,
      COALESCE((SELECT SUM(nds.capped_count) FROM new_daily_shares nds WHERE nds.user_id = p.id), 0) AS new_shares,
      COALESCE((SELECT SUM(ndf.capped_count) FROM new_daily_friends ndf WHERE ndf.user_id = p.id), 0) AS new_friends,
      COALESCE((SELECT SUM(ndl.capped_count) FROM new_daily_livestreams ndl WHERE ndl.user_id = p.id), 0) AS new_livestreams
    FROM profiles p
  )
  SELECT 
    p.id,
