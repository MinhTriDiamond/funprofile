  ...
  ELSIF p_type = 'livestreams' THEN
    RETURN QUERY
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE c.live_session_id = ls.id AND c.status = 'ready' ORDER BY c.created_at DESC LIMIT 1
      ) cr ON true
    ...
    RETURN QUERY
      WITH reward_activities AS (
        ...
        SELECT la.id::uuid, '⭐ PPLP Mint: +' || COALESCE(la.mint_amount, 0)::text || ' CAMLY',
               NULL::text, NULL::text, NULL::jsonb, 'reward_pplp'::text, la.created_at
      ...
      FROM reward_activities ra ORDER BY ra.created_at DESC;
  END IF;
END;
$$;
