...
-- 1. Add content_hash and is_reward_eligible columns to posts table
ALTER TABLE public.posts 
ADD COLUMN IF NOT EXISTS content_hash TEXT,
ADD COLUMN IF NOT EXISTS is_reward_eligible BOOLEAN DEFAULT true;

-- 2. Create index for fast duplicate lookup
CREATE INDEX IF NOT EXISTS idx_posts_user_content_hash 
ON public.posts (user_id, content_hash) 
WHERE content_hash IS NOT NULL;

-- 3. Update get_user_rewards_v2 to only count reward-eligible posts
