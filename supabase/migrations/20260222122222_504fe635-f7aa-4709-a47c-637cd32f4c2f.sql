...
CREATE OR REPLACE FUNCTION public.get_app_stats()
 RETURNS TABLE(total_users bigint, total_posts bigint, total_photos bigint, total_videos bigint, total_rewards bigint, total_camly_circulating numeric, treasury_received numeric, total_camly_claimed numeric)
 LANGUAGE plpgsql
 STABLE
 SECURITY DEFINER
...
BEGIN
  ...
  circulating AS (
    SELECT COALESCE(SUM(amount::NUMERIC), 0) AS camly_circ
    FROM (
      SELECT amount FROM reward_claims
      UNION ALL
      SELECT amount FROM donations WHERE token_symbol = 'CAMLY' AND status = 'confirmed'
    ) combined
  ),
  treasury AS (
