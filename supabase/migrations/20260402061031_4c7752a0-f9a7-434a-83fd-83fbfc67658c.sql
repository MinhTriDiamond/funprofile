...
-- 1. Add btc_address to profiles
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS btc_address TEXT;
CREATE INDEX IF NOT EXISTS idx_profiles_btc_address ON public.profiles(btc_address) WHERE btc_address IS NOT NULL;

-- 2. Update normalize_wallet_addresses to include btc_address
...
-- 3. Expand donations table for Bitcoin support
ALTER TABLE public.donations ADD COLUMN IF NOT EXISTS chain_family TEXT NOT NULL DEFAULT 'evm';
ALTER TABLE public.donations ADD COLUMN IF NOT EXISTS fee TEXT;
ALTER TABLE public.donations ADD COLUMN IF NOT EXISTS confirmations INTEGER DEFAULT 0;
ALTER TABLE public.donations ADD COLUMN IF NOT EXISTS block_height BIGINT;

-- 4. Expand wallet_transfers table for Bitcoin support
ALTER TABLE public.wallet_transfers ADD COLUMN IF NOT EXISTS chain_family TEXT NOT NULL DEFAULT 'evm';
ALTER TABLE public.wallet_transfers ADD COLUMN IF NOT EXISTS fee TEXT;
ALTER TABLE public.wallet_transfers ADD COLUMN IF NOT EXISTS confirmations INTEGER DEFAULT 0;

-- 5. Update get_user_donation_summary to include BTC
