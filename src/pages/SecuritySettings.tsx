...
const SecuritySettings = () => {
  ...
  // Force refetch user on mount to detect email verification from other tabs
  useEffect(() => {
  ...
  // Listen for USER_UPDATED to detect email verification
  useEffect(() => {
  ...
  if (authLoading || loginMethods.isLoading) {
    return (
      <div className="min-h-screen overflow-hidden">
        <FacebookNavbar />
        <main className="fixed inset-x-0 top-[3cm] bottom-0 flex items-center justify-center">
  ...
  const securityProgress = Math.min(100, Math.round((activeMethodCount / 3) * 100));
  ...
  // Next action after set password
    ...
    <div className="min-h-screen overflow-hidden">
      ...
      <main data-app-scroll className="fixed inset-x-0 bottom-0 top-[3cm] overflow-y-auto pb-20 lg:pb-4">
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
              ...
              <div>
                ...
                {isFullySecured ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    {activeMethodCount}/3 phương thức đang hoạt động
                  </p>
                )}
              </div>
            </div>
          ...
          {/* Recommended Next Step — only show when not fully secured */}
          {recommendedAction && recommendedConfig[recommendedAction] && (
            <div className="rounded-xl p-4 bg-primary/5 border border-primary/20">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                {recommendedConfig[recommendedAction].text}
              </p>
              <Button
                size="sm"
                onClick={() => handleNextAction(recommendedAction)}
              >
      ...
      <LinkEmailDialog
        open={showLinkEmail}
        onOpenChange={setShowLinkEmail}
      />
      <LinkWalletDialog
        open={showLinkWallet}
        onOpenChange={setShowLinkWallet}
      />
    </div>
  );
};
