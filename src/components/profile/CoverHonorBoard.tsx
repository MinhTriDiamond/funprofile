...
export const CoverHonorBoard = ({ userId, username, avatarUrl }: CoverHonorBoardProps) => {
  ...
  const StatRow = ({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) => (
    <div className="flex items-center justify-between py-1 px-2 sm:px-2.5 rounded-lg border border-[#C9A84C]/50 bg-gradient-to-r from-[#166534] to-[#14532d] backdrop-blur-sm">
      <div className="flex items-center gap-1 sm:gap-1.5">
        <div className="text-[#C9A84C]">
          {icon}
        </div>
        <span className="text-[#C9A84C] font-bold text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wide">
          {label}
        </span>
      </div>
  ...
  return (
    <>
      ...
      <div className="hidden md:block absolute right-3 sm:right-4 top-3 sm:top-4 bottom-3 sm:bottom-4 w-[50%] max-w-[500px] min-w-[300px]">
        ...
        <div className="h-full rounded-2xl overflow-hidden border border-[#C9A84C]/60 flex flex-col bg-gradient-to-br from-[#166534] via-[#14532d] to-[#0f3d22]">
            ...
            <div className="text-center mb-3">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <img 
                  ...
                  className="w-9 h-9 sm:w-11 sm:h-11 rounded-full border border-[#C9A84C]/50 shadow-[0_4px_12px_rgba(201,168,76,0.3)]"
                />
              ...
              <div className="flex items-center justify-center gap-2">
                <Avatar className="w-8 h-8 border border-[#C9A84C]/60 shadow-[0_2px_8px_rgba(201,168,76,0.3)]">
                  <AvatarImage src={avatarUrl} />
                  <AvatarFallback className="bg-gradient-to-br from-[#C9A84C] to-[#a88a3d] text-[#166534] font-bold text-sm">
...
export const MobileStats = ({ userId, username, avatarUrl }: MobileStatsProps) => {
  ...
  return (
    <div className="w-full">
      <div className="rounded-xl overflow-hidden border border-[#C9A84C]/60 bg-gradient-to-br from-[#166534] via-[#14532d] to-[#0f3d22]">
        <div className="p-3">
          {/* Header with user info */}
          <div className="flex items-center justify-center gap-2 mb-3">
            <Avatar className="w-8 h-8 border border-[#C9A84C]/60">
              <AvatarImage src={avatarUrl} />
              <AvatarFallback className="bg-gradient-to-br from-[#C9A84C] to-[#a88a3d] text-[#166534] font-bold text-sm">
                {username?.[0]?.toUpperCase() || 'U'}
              </AvatarFallback>
          ...
          <div className="grid grid-cols-4 gap-1.5 text-center mb-2">
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <ArrowUp className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.posts_count)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Posts</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Star className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.reactions_on_posts)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Reactions</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <MessageCircle className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.comments_count)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Comments</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Users className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.friends_count)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Friends</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1.5 text-center mb-2">
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Share2 className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.shares_count)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Shares</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Video className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.livestreams_count)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Live</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Gift className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.claimable)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Claimable</div>
            </div>
            <div className="bg-[#14532d]/80 rounded-lg py-1.5 px-1 border border-[#C9A84C]/40">
              <Coins className="w-3.5 h-3.5 mx-auto text-[#C9A84C] mb-0.5" />
              <div className="text-white font-bold text-xs">{formatNumber(stats.claimed)}</div>
              <div className="text-[#C9A84C]/80 text-[8px] uppercase">Claimed</div>
            </div>
          </div>
          
          {/* Total rows */}
          <div className="grid grid-cols-2 gap-1.5">
            <div className="bg-[#C9A84C]/15 rounded-lg py-1.5 px-2 border border-[#C9A84C]/50 flex items-center justify-between">
              <div className="flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-[#C9A84C]" />
                <span className="text-[#C9A84C] font-bold text-[9px] uppercase">Today</span>
              </div>
              <span className="text-white font-bold text-xs">{formatNumber(stats.today_reward)}</span>
            </div>
            <div className="bg-[#C9A84C]/15 rounded-lg py-1.5 px-2 border border-[#C9A84C]/50 flex items-center justify-between">
              <div className="flex items-center gap-1">
                <BadgeDollarSign className="w-3.5 h-3.5 text-[#C9A84C]" />
                <span className="text-[#C9A84C] font-bold text-[9px] uppercase">Total</span>
              </div>
