/**
 * GiftRecipientSearch — Extracted from UnifiedGiftSendDialog
 * Handles recipient search (by username or wallet address) and multi-select chips.
 */
...
interface GiftRecipientSearchProps {
  resolvedRecipients: ResolvedRecipient[];
  onAddRecipient: (r: ResolvedRecipient) => void;
...
function resolveWalletAddress(profile: {
...
export function GiftRecipientSearch({
  resolvedRecipients,
  onAddRecipient,
  onRemoveRecipient,
  onClearAll,
  ...
  }, [debouncedSearchQuery, searchTab, isOpen, performSearch]);
      ...
      {resolvedRecipients.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          ...
          {resolvedRecipients.length > 1 && (
            <button type="button" onClick={onClearAll} className="text-xs text-destructive hover:underline px-2 py-1">
              Xóa tất cả
            </button>
          )}
        </div>
