...
export const WalletLogin = ({ onSuccess }: WalletLoginProps) => {
  ...
  const checkMetaMask = () => {
    if (typeof window.ethereum === 'undefined' || !window.ethereum.isMetaMask) {
      toast.error(t('walletNotInstalled'));
    ...
    <div className="space-y-6">
      ...
      {step === 'connect' && (
        <>
          <div className="text-center space-y-2">
            <h3 className="text-lg font-bold text-slate-800">{t('walletConnect')}</h3>
          ...
          >
            {t('walletInstallPrompt')} <ExternalLink size={14} />
          </button>
        </>
      )}

      {step === 'sign' && walletAddress && (
        <>
          <div className="text-center space-y-2">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-100 mb-2">
              <CheckCircle2 className="text-emerald-600" size={28} />
            </div>
            <p className="text-slate-600">{t('walletConnected')}</p>
            <p className="font-mono text-sm bg-slate-100 px-3 py-2 rounded-lg inline-block">
      ...
      {step === 'verify' && (
        <div className="text-center space-y-4 py-8">
          ...
          <p className="text-slate-600 font-medium">{t('walletVerifying')}</p>
        </div>
      )}
    </div>
  );
