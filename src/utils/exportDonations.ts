...
import { DonationRecord as PublicDonationRecord } from '@/hooks/usePublicDonationHistory';
...
/**
 * Export public donation records (from usePublicDonationHistory) to PDF
 */
export async function exportPublicDonationsToPDF(
  ...
  const date = new Date().toISOString().split('T')[0];
  const titleMap = { sent: 'Lịch Sử Giao Dịch Đã Gửi', received: 'Lịch Sử Giao Dịch Đã Nhận', all: 'Lịch Sử Giao Dịch' };
    ...
    <table style="width:100%;border-collapse:collapse;font-size:11px;">
      ...
      <tbody>
        ${donations.map((d, i) => `
          <tr style="background:${i % 2 === 0 ? '#f0fdf4' : '#fff'};border-bottom:1px solid #e5e7eb;">
            <td style="padding:6px 8px;">${escapeHtml(formatDate(d.created_at))}</td>
            <td style="padding:6px 8px;">@${escapeHtml(d.sender_username || d.sender_display_name || 'Unknown')}</td>
            <td style="padding:6px 8px;">@${escapeHtml(d.recipient_username || d.recipient_display_name || 'Unknown')}</td>
            <td style="padding:6px 8px;text-align:right;font-weight:bold;color:#10b981;">${escapeHtml(String(d.amount))}</td>
            <td style="padding:6px 8px;">${escapeHtml(d.token_symbol)}</td>
            <td style="padding:6px 8px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(d.message || '')}</td>
            <td style="padding:6px 8px;">${escapeHtml(translateStatus(d.status))}</td>
